...
export async function POST(request) {
  try {
    ...
    if (
      ...
      throw new CheckoutError(
        "Choose UPI or Card for online payment."
      );
    }

    const checkoutType =
      body.checkoutType === "BUY_NOW"
        ? "BUY_NOW"
        : body.checkoutType === "CART"
        ? "CART"
        : null;
     ...
     * IMPORTANT:
     *
     * We never trust the amount sent
     * by the browser.
     *
     * prepareOnlineCheckout reads the
     * actual product/cart data from D1
     * and calculates the amount itself.
     */
    const payment =
