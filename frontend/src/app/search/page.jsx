    ...
    <main className="min-h-screen overflow-x-hidden bg-[#FBF5EA]">
        ...
        <div className="flex flex-col gap-4 border-b border-[#D8CCB4] pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex w-full items-center gap-2 sm:w-auto sm:shrink-0">
            <button type="button" onClick={() => setIsFilterOpen(true)} className="inline-flex h-11 flex-1 items-center justify-center gap-2 border border-[#BDA985] bg-[#FFFDF9] px-4 text-sm font-medium text-[#2B2721] sm:flex-none sm:min-w-28">
              <SlidersHorizontal size={19} /><span>Filter</span>{selectedCategories.length > 0 && <span className="rounded-full bg-[#D39A2F] px-1.5 text-xs text-white">{selectedCategories.length}</span>}
            </button>
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 z-[80] flex items-end bg-[#2F2B27]/40 sm:items-center sm:justify-center" role="dialog" aria-modal="true" aria-label="Filter search results">
          ...
          <div className="relative flex max-h-[88dvh] w-full flex-col rounded-t-3xl bg-[#FFFDF9] shadow-2xl sm:max-w-md sm:rounded-2xl">
            <div className="flex items-center justify-between border-b border-[#E8DCC8] px-5 py-4">
              ...
              <button type="button" aria-label="Close filters" onClick={() => setIsFilterOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-full hover:bg-[#F8F2E8]"><X size={22} /></button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
              <p className="text-xs font-medium uppercase tracking-[0.13em] text-[#8A7A65]">Category</p>
              <div className="mt-3 space-y-2">
                {categories.map((category) => <label key={category} className="flex min-h-11 items-center gap-3 rounded-xl px-2 text-sm text-[#2B2721] hover:bg-[#F8F2E8]"><input type="checkbox" checked={selectedCategories.includes(category)} onChange={() => toggleCategory(category)} className="h-5 w-5 accent-[#C79A2B]" /><span>{category}</span></label>)}
              </div>
            </div>
